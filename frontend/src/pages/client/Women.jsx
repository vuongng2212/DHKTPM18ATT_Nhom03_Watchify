...
const WomenPage = () => {
  ...
  }, [currentPage, selectedBrands, priceRange, sortBy, sortDirection, brandSlug, womenCategoryId]);
  ...
  const fetchWomenCategory = async () => {
    try {
      ...
      const womenCategory = mainCategories.find(cat => cat.slug === 'dong-ho-nu');
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Filter by Women category using categoryId
      if (womenCategoryId) {
        params.categoryId = womenCategoryId;
      }
    ...
    return (
      <div className="container mt-4 mb-20 mx-auto px-4">
        <p className="flex justify-center text-gray-600 text-3xl font-bold mt-24">
          Đồng hồ nữ đẹp chính hãng, cao cấp, mẫu mới 2025, góp 0%
        </p>
        <img
          src={banner}
          alt="Banner"
  ...
  return (
    <div className="container mt-4 mb-20 mx-auto px-4">
      ...
      <p className="flex justify-center text-gray-400 text-xl mb-8 px-4">
        Những mẫu đồng hồ nữ đẹp luôn là món phụ kiện thời trang hoàn hảo cho
        tất cả các dịp, giúp nữ giới tự tin hơn – khẳng định phong cách...
      </p>
